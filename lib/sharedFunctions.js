...
module.exports = function sharedFunctions(knex) {
  ...
  return {
    ...
    objectCreationAdmin: function(adminName, adminEmail, pollTitle, pollDescription, choiceTitle, choiceDescription, endDate, requireName){
      console.log('Entered objectCreationAdmin')
      startDate = Date.now();
      urlString = urlString();
      const pollCreatorInfo = {[adminName, adminEmail]};
      const pollInfo = {[pollTitle, pollDescription,startDate,endDate, urlString, requireName]};

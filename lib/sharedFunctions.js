...
module.exports = function sharedFunctions(knex) {
  ...
  return {
    ...
    sumTime: function (date, time, ampm){
      epochAmPm = 43200;
      epochDate = moment(date).valueOf();
      epochTime = moment(time).valueOf();
      if (ampm === 'pm') {
        console.log(epochAmPm+epochDate+epochTime)
        return epochAmPm+epochDate+epochTime;
      }
      console.log(epochDate + epochTime);
      return epochDate + epochTime;
    }
    ,
